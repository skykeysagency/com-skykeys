...
function layoutApts(dayApts: any[]) {
...
// ── Sub-components defined OUTSIDE parent to prevent React #185 ──
...
interface TimeGridProps {
  ...
  nowMinute: Date;
  onSlotClick: (e: React.MouseEvent<HTMLDivElement>, day: Date) => void;
  onAptClick: (id: string) => void;
  onDayClick: (day: Date) => void;
}

function TimeGrid({ cols, scrollRef, appointments, busySlots, nowMinute, onSlotClick, onAptClick, onDayClick }: TimeGridProps) {
  ...
  const aptsForDay = (day: Date) => appointments.filter((a) => isSameDay(parseISO(a.start_at), day));
                ...
                {dayApts.map((apt) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={(e) => { e.stopPropagation(); onAptClick(apt.id); }}
                    >
...
function MonthView({ days, currentDate, appointments, onDayClick, onAptClick }: MonthViewProps) {
  const aptsForDay = (day: Date) => appointments.filter((a) => isSameDay(parseISO(a.start_at), day));
...
function TodaySummary({ viewMode, currentDate, appointments, onAptClick }: TodaySummaryProps) {
  if (viewMode !== "day") return null;
  const todayApts = appointments.filter((a) => isSameDay(parseISO(a.start_at), currentDate));
      ...
      {todayApts.map((apt) => {
        ...
        return (
          <span
            ...
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border cursor-pointer ${color.bg} ${color.text}`}
          >
